...
const CONFIG = {
  preshift: {
    ...
    textModeIndex: 10,
    shiftSettingsKey: 'preshift',
    fallbackTimes: {
      PAGI: { briefingTime: '22.30 UTC', incomingTime: '23.30 UTC', outgoingTime: '00.00 UTC' },
    ...
    section1Title: 'Informasi Briefing',
    idPrefix: 'BRIEF',
    btnText: 'Simpan Briefing'
  },
  postshift: {
    checklistItems: [
    ...
    textModeIndex: 6,
    shiftSettingsKey: 'postshift',
    fallbackTimes: {
      PAGI: { briefingTime: '05.30 UTC', incomingTime: '05.30 UTC', outgoingTime: '06.00 UTC' },
    ...
    section1Title: 'Informasi Post-Shift',
    idPrefix: 'POST',
    btnText: 'Simpan Briefing'
  }
};
...
function initChecklist(typeConfig) {
  return typeConfig.checklistItems.map(item => ({
    no: item.no,
    subject: item.subject,
    details: item.details,
    checked: true,
    remarks: item.defaultText || '',
    isTextMode: item.no === typeConfig.textModeIndex,
  }));
}
...
function ChecklistDropdown({ checked, onChange, isTextMode, onSelectKeterangan }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', width: '100%' }}>
      ...
      >
        {checked ? (isTextMode ? (isTextMode === 'BY SUPERVISOR ON DUTY' ? 'BY SUPERVISOR ON DUTY' : 'BY SUPERVISOR') : <Check size={22} strokeWidth={3} />) : '✎ Keterangan'}
      </button>
      ...
      {isOpen && (
        <div style={{
          ...
          >
            {isTextMode ? (isTextMode === 'BY SUPERVISOR ON DUTY' ? 'BY SUPERVISOR ON DUTY' : 'BY SUPERVISOR') : <><Check size={18} strokeWidth={3} /> Centang</>}
          </button>
          <button
            type="button"
      ...
      <div style={{ padding: '1rem' }}>
        <div style={{ marginBottom: '0.75rem' }}>
          <CustomSelect
            ...
            onChange={onChange}
            options={users.filter(u => u.role === 'admin').map(u => ({ value: u.initial, label: `${u.initial} – ${u.nama}` }))}
          />
        </div>
        ...
        <div>
          <label style={{ display: 'block', fontWeight: 600, fontSize: '0.78rem', color: '#475569', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Waktu (UTC)</label>
          <input type="text" value={time} readOnly disabled style={{ width: '100%', boxSizing: 'border-box', padding: '0.65rem 0.75rem', borderRadius: 8, border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontSize: '0.9rem', fontWeight: 600, cursor: 'not-allowed', outline: 'none' }} />
        </div>
      </div>
    </div>
  );
...
export default function ShiftForm({ type }) {
  ...
  useEffect(() => {
    if (shiftSettings && shiftSettings[config.shiftSettingsKey] && shiftSettings[config.shiftSettingsKey][shift]) {
      ...
      setOutgoingTime(shiftSettings[config.shiftSettingsKey][shift].outgoing || '');
    } else {
      const fb = config.fallbackTimes[shift];
      if (fb) {
        setBriefingTime(fb.briefingTime);
        setIncomingTime(fb.incomingTime);
  ...
  const handleSubmit = async (e) => {
    ...
    const finalChecklist = [
      ...
      ...extraItems.map((it, i) => ({
        no: config.checklistItems.length + 1 + i,
        subject: it.subject,
        details: it.details,
        checked: it.checked,
    ...
    try {
      ...
      if (data.success) {
        navigate('/admin/dashboard', { state: { tab: config.navTab } });
      } else {
        setError(data.error || 'Gagal menyimpan');
      }
    } catch { 
      setError(type === 'postshift' ? 'Gagal membuat post-shift: Failed to fetch' : 'Terjadi kesalahan server'); 
    }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', padding: '0 0.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <div>
            ...
            <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.85rem', color: '#64748b' }}>{config.subtitle}</p>
          </div>
        </div>
      </div>
      ...
      <form id="briefing-form" onSubmit={handleSubmit} onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', padding: '1.5rem', marginBottom: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '1.1rem', paddingBottom: '0.75rem', borderBottom: '1px solid #f1f5f9' }}>
            ...
            <h2 style={{ margin: 0, fontWeight: 700, fontSize: '1rem', color: '#1e293b' }}>{config.section1Title}</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr', gap: '1.25rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 600, fontSize: '0.78rem', color: '#475569', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Waktu Briefing (UTC)</label>
              <input type="text" value={briefingTime} readOnly disabled style={{ width: '100%', boxSizing: 'border-box', padding: '0.65rem 0.75rem', borderRadius: 8, border: '1px solid #e2e8f0', background: '#f8fafc', color: '#64748b', fontSize: '0.9rem', fontWeight: 600, cursor: 'not-allowed', outline: 'none' }} />
            </div>
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.87rem' }}>
              ...
              <tbody>
                {checklist.map((item, idx) => {
                  const defaultRemarkText = item.isTextMode ? (config.checklistItems.find(i => i.no === item.no)?.defaultText || 'BY SUPERVISOR') : '';
                  return (
                      ...
                      {item.isTextMode ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <ChecklistDropdown
                            ...
                            isTextMode={defaultRemarkText}
                            onChange={(val) => {
                              setChecklist(prev => {
                                const u = [...prev];
                                u[idx] = { ...u[idx], checked: val, remarks: val ? defaultRemarkText : '' };
                                return u;
                              });
                            }}
                            onSelectKeterangan={() => {}}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2rem' }}>
          ...
          >
            <Save size={18} />
            {loading ? 'Menyimpan...' : config.btnText}
          </button>
        </div>
